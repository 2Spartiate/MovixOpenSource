...
test('TV launcher uses TV-specific foreground framing while phone stays frozen', async () => {
  ...
  for (const edge of ['left', 'top', 'right', 'bottom']) {
    ...
    assert.match(tvForeground, new RegExp(`android:${edge}="28dp"`));
  }
...
test('television uiMode overrides the application icon foreground without changing phone resources', async () => {
  const [phoneForeground, tvForeground, launcher, round] = await Promise.all([
    bytes('android/app/src/main/res/drawable/ic_launcher_foreground_scaled.xml').then(buffer => buffer.toString('utf8')),
    bytes('android/app/src/main/res/drawable-television/ic_launcher_foreground_scaled.xml').then(buffer => buffer.toString('utf8')),
    bytes('android/app/src/main/res/mipmap-anydpi-v26/ic_launcher.xml').then(buffer => buffer.toString('utf8')),
    bytes('android/app/src/main/res/mipmap-anydpi-v26/ic_launcher_round.xml').then(buffer => buffer.toString('utf8')),
  ]);
